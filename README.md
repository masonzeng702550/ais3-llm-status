...
每 5 分鐘從外部對每個模型送出一次真實推論請求，記錄可用性與延遲，並以純靜態網站呈現。
...
約 10 次的軟性上限，每 5 分鐘部署一次會直接撞上；改由瀏覽器端讀取資料分支後，網站只在
程式碼或事件公告變動時才重建。
...
| `raw/YYYY-MM-DD.jsonl` | 原始探測紀錄，保留 30 天 |
| `badges/<元件>.svg` | 狀態徽章 |
